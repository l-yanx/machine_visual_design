#!/usr/bin/env python3
"""A->B 变换、独立 ORB 检测、相似性匹配、拼接连线可视化。"""
...
def normalize_output_path(p: Path) -> Path:
    if p.suffix.lower() in _IMAGE_SUFFIXES:
        return p
    return p / "orb_match_stitch_out.png"
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="ORB pipeline: transform, detect, match, stitch")
    ...
    out_path = args.output if args.output is not None else args.result_dir / "orb_match_stitch_out.png"
    ...
    a1_path = args.result_dir / "a1_orb.png"
    b1_path = args.result_dir / "b1_orb.png"
    cv2.imwrite(str(a1_path), a1)
    cv2.imwrite(str(b1_path), b1)

    matches = match_orb_descriptors(desc_a, desc_b, ratio=args.ratio)

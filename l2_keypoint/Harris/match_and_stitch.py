...
def match_descriptors(
    ...
    """Lowe ratio + mutual NN，输出原始角点下标对。"""
    ...
    for i in range(dist.shape[0]):
        ...
        d0 = float(dist[i, j0])
        if nb >= 2:
            d1 = float(dist[i, int(order[1])])
            if d0 >= ratio * (d1 + 1e-8):
                continue
        candidates.append((i, j0))

    nearest_i = np.argmin(dist, axis=0)
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Harris pipeline: transform, detect, match, stitch")
    ...
    out_path = args.output if args.output is not None else args.result_dir / "match_stitch_out.png"
    ...
    a1_path = args.result_dir / "a1_corners.png"
    b1_path = args.result_dir / "b1_corners.png"
    ...
    matches = match_descriptors(desc_a, desc_b, idx_a, idx_b, ratio=args.ratio)
    ...
    print(f"corners A={len(pts_a)}, B={len(pts_b)}, matches={len(matches)}")
    return 0

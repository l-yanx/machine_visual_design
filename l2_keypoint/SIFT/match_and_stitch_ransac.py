#!/usr/bin/env python3
"""A->B 变换、独立 SIFT 检测、Ratio+RANSAC 匹配、拼接连线可视化。"""
...
def normalize_output_path(p: Path) -> Path:
    if p.suffix.lower() in _IMAGE_SUFFIXES:
        return p
    return p / "sift_match_stitch_ransac_out.png"
...
def ratio_match_indices(desc_a: np.ndarray, desc_b: np.ndarray, ratio: float) -> list[tuple[int, int]]:
    """SIFT 描述子做 KNN + ratio，返回 (idx_a, idx_b)。"""
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="SIFT pipeline: ratio+RANSAC match and stitch")
    ...
    out_path = args.output if args.output is not None else args.result_dir / "sift_match_stitch_ransac_out.png"
    ...
    a1_path = args.result_dir / "a1_sift_ransac.png"
    b1_path = args.result_dir / "b1_sift_ransac.png"

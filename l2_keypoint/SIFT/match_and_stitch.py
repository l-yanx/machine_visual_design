#!/usr/bin/env python3
"""A->B 变换、独立 SIFT 检测、相似性匹配、拼接连线可视化。"""
...
from sift_detector import detect_sift_features, draw_keypoints
...
def normalize_output_path(p: Path) -> Path:
    if p.suffix.lower() in _IMAGE_SUFFIXES:
        return p
    return p / "sift_match_stitch_out.png"
...
def match_sift_descriptors(
    ...
    """KNN + Lowe ratio + mutual NN 匹配。"""
    if desc_a.shape[0] == 0 or desc_b.shape[0] == 0:
        return []

    bf = cv2.BFMatcher(cv2.NORM_L2, crossCheck=False)
    ab = bf.knnMatch(desc_a, desc_b, k=2)
    ba = bf.knnMatch(desc_b, desc_a, k=2)
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="SIFT pipeline: transform, detect, match, stitch")
    ...
    parser.add_argument("--nfeatures", type=int, default=1200, help="number of SIFT keypoints")
    parser.add_argument("--contrast-threshold", type=float, default=0.04, help="SIFT contrast threshold")
    parser.add_argument("--edge-threshold", type=float, default=10.0, help="SIFT edge threshold")
    parser.add_argument("--sigma", type=float, default=1.6, help="SIFT Gaussian sigma")
    parser.add_argument("--ratio", type=float, default=0.75, help="Lowe ratio threshold")
    args = parser.parse_args()

    args.result_dir.mkdir(parents=True, exist_ok=True)
    out_path = args.output if args.output is not None else args.result_dir / "sift_match_stitch_out.png"
    ...
    pts_a, desc_a = detect_sift_features(
        gray_a,
    ...
    pts_b, desc_b = detect_sift_features(
        gray_b,
        nfeatures=args.nfeatures,
        contrast_threshold=args.contrast_threshold,
        edge_threshold=args.edge_threshold,
    ...
    a1_path = args.result_dir / "a1_sift.png"
    b1_path = args.result_dir / "b1_sift.png"
    cv2.imwrite(str(a1_path), a1)
    cv2.imwrite(str(b1_path), b1)

    matches = match_sift_descriptors(desc_a, desc_b, ratio=args.ratio)
    stitched = stitch_and_draw_matches(a1, b1, pts_a, pts_b, matches)
    ...
    print(f"keypoints A={len(pts_a)}, B={len(pts_b)}, matches={len(matches)}")
    return 0
